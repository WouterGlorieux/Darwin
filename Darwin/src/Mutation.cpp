...
std::string DoubleMutation::Gaussian(double sigma = 1.0){
	...
	//make sure new value is not smaller than min boundary
	if(m_ChromosomeNode->first_attribute("min")){
		if( dValue < atoi(m_ChromosomeNode->first_attribute("min")->value()) ){
			return m_strValue;
		}
	}
	...
	//make sure new value is not greater than max boundary
	if(m_ChromosomeNode->first_attribute("max")){
		if( dValue > atoi(m_ChromosomeNode->first_attribute("max")->value()) ){
			return m_strValue;
		}
	}
...
std::string DoubleMutation::Insertion(){
	...
	if(m_ChromosomeNode->first_attribute("max")){
		int nMin = atoi(m_ChromosomeNode->first_attribute("min")->value());
...
std::string AlphanumMutation::Insertion(){
	...
	if(m_ChromosomeNode->first_attribute("maxChars")){
		int nChars = atoi(m_ChromosomeNode->first_attribute("maxChars")->value());
