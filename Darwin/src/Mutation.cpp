...
std::string BitMutation::Uniform(){
	...
	return cBinaryEncoding.RandomData(m_Node->parent());
}
...
std::string BitMutation::Duplication(){
	...
	if(m_Node->parent()->first_attribute("bits")){
		int bits = atoi(m_Node->parent()->first_attribute("bits")->value());
...
std::string IntegerMutation::BitString(){
	...
	if(strValue.size() > 0){
		...
		if(m_Node->parent()->first_attribute("min")){
			if( atoi(strValue.c_str()) < atoi(m_Node->parent()->first_attribute("min")->value()) ){
				return m_Node->value();
			}
		}

		//make sure new value is not greater than max boundary
		if(m_Node->parent()->first_attribute("max")){
			if( atoi(strValue.c_str()) > atoi(m_Node->parent()->first_attribute("max")->value()) ){
				return m_Node->value();
			}
		}
...
std::string IntegerMutation::Boundary(){
	...
	if(m_Node->parent()->first_attribute("min") && m_Node->parent()->first_attribute("max")){
		//pick one of the 2 boundaries at random
		int nRandom = rand() % 2;
		(nRandom == 0) ? ss << m_Node->parent()->first_attribute("min")->value() : ss << m_Node->parent()->first_attribute("max")->value();
		return ss.str();//return a string with the contents of the stream
	}
	else if(m_Node->parent()->first_attribute("min")){
		 ss << m_Node->parent()->first_attribute("min")->value();
		return ss.str();//return a string with the contents of the stream
	}
	else if(m_Node->parent()->first_attribute("max")){
		ss << m_Node->parent()->first_attribute("max")->value();
		return ss.str();//return a string with the contents of the stream
	}
	else
	{
...
std::string IntegerMutation::Uniform(){
	...
	return cIntegerEncoding.RandomData(m_Node->parent());
}
std::string IntegerMutation::Gaussian(double sigma = 1.0){
	...
	//make sure new value is not smaller than min boundary
	if(m_Node->parent()->first_attribute("min")){
		if( nValue < atoi(m_Node->parent()->first_attribute("min")->value()) ){
			return m_Node->value();
		}
	}
	...
	//make sure new value is not greater than max boundary
	if(m_Node->parent()->first_attribute("max")){
		if( nValue > atoi(m_Node->parent()->first_attribute("max")->value()) ){
			return m_Node->value();
		}
	}
...
std::string DoubleMutation::BitString(){
	...
	if(strValue.size() > 0){
		...
		//make sure new value is not smaller than min boundary
		if(m_Node->parent()->first_attribute("min")){
			if( atof(strValue.c_str()) < atof(m_Node->parent()->first_attribute("min")->value()) ){
				return m_Node->value();
			}
		}
		...
		//make sure new value is not greater than max boundary
		if(m_Node->parent()->first_attribute("max")){
			if( atof(strValue.c_str()) > atof(m_Node->parent()->first_attribute("max")->value()) ){
				return m_Node->value();
			}
		}
	}
...
std::string DoubleMutation::Uniform(){
	...
	return cDoubleEncoding.RandomData(m_Node->parent());
}
std::string DoubleMutation::Gaussian(double sigma = 1.0){
	...
	//make sure new value is not smaller than min boundary
	if(m_Node->parent()->first_attribute("min")){
		if( dValue < atoi(m_Node->parent()->first_attribute("min")->value()) ){
			return m_Node->value();
		}
	}
	...
	//make sure new value is not greater than max boundary
	if(m_Node->parent()->first_attribute("max")){
		if( dValue > atoi(m_Node->parent()->first_attribute("max")->value()) ){
			return m_Node->value();
		}
	}
	...
	if(m_Node->parent()->first_attribute("decimals")){
		int nDecimals = atoi(m_Node->parent()->first_attribute("decimals")->value());
...
std::string DoubleMutation::Insertion(){
	...
	if(m_Node->parent()->first_attribute("max")){
		int nMin = atoi(m_Node->parent()->first_attribute("min")->value());
...
std::string AlphanumMutation::Uniform(){
	AlphanumEncoding cAlphanumEncoding;
	return cAlphanumEncoding.RandomData(m_Node->parent());
}
...
std::string AlphanumMutation::Duplication(){
	...
	if(m_Node->parent()->first_attribute("maxChars")){
		int chars = atoi(m_Node->parent()->first_attribute("maxChars")->value());
...
std::string AlphanumMutation::Insertion(){
	...
	if(m_Node->parent()->first_attribute("maxChars")){
		int nChars = atoi(m_Node->parent()->first_attribute("maxChars")->value());
...
std::string CustomMutation::Uniform(){
	CustomEncoding cCustomEncoding;
	cCustomEncoding.SetChars(m_Node->parent()->first_attribute("chars")->value());
	return cCustomEncoding.RandomData(m_Node->parent());
}
...
std::string ChromosomeMutation::Duplication(){
	...
	if(nGenes){
		...
		for(int i = nBegin; i <= nEnd; i++){
			geneNode = Mutation::GetGeneNode(i);
			clonedNode = m_Node->document()->clone_node(geneNode);

			geneNode = Mutation::GetGeneNode(nEnd+j+1);
...
std::string ChromosomeMutation::Deletion(){
	...
	if(nGenes){
		...
		for(int i = nBegin; i <= nEnd; i++){
			geneNode = Mutation::GetGeneNode(nBegin);
...
std::string ChromosomeMutation::Insertion(){
	...
	for (rapidxml::xml_node<>* geneNode = m_Node->first_node("Gene"); geneNode; geneNode = geneNode->next_sibling("Gene")){
		...
		geneNode = Mutation::GetGeneNode(nRandom1);
...
std::string ChromosomeMutation::Swap(){
	...
	if(nGenes){
		...
		geneNode = Mutation::GetGeneNode(nRandom2);
		clonedNode = m_Node->document()->clone_node(geneNode);
		geneNode = Mutation::GetGeneNode(nRandom1+1);
		m_Node->insert_node(geneNode, clonedNode);

		//copy the first node just after the second node
		geneNode = Mutation::GetGeneNode(nRandom1);
		clonedNode = m_Node->document()->clone_node(geneNode);
		geneNode = Mutation::GetGeneNode(nRandom2+2);
		m_Node->insert_node(geneNode, clonedNode);
